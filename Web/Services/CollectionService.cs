...
public class CollectionService : ICollectionService
{
    ...
    public CollectionService(AppDbContext db, ICollectionConfigService cfg, IInventoryService inv)
    {
        _db = db; _cfg = cfg; _inv = inv;
    }
    ...
    public async Task<(bool success, string message, int? collectionId)> ObtainAsync(long userId, CancellationToken ct = default)
    {
        ...
        if (items.Count == 0) return (false, "暂无可用藏品", null);
        ...
        if (candidates.Count == 0) return (false, "藏品数量不足", null);
        ...
        // 记录玩家获得 & 更新全局计数（若有限量）
        var existing = await _db.Set<PlayerCollection>().AnyAsync(x => x.UserId == userId && x.CollectionId == chosen.ID, ct);
        if (!existing)
        {
